trigger OppLineBeforeInsert on OpportunityLineItem (before insert) {
	...
	system.debug('****************************** oppId.size : '+myOppLines.size()+'*******************************');
	 ...
	 for(OpportunityLineItem myOppLine : myOppLines) {
		PricebookEntry[] myPricebookEntries = [Select p.Product2.Administratie__c,p.Product2.Name,p.Product2.Description,p.Product2Id, p.Name, p.Id From PricebookEntry p  where Id =: myOppLine.PricebookEntryId];
		PricebookEntry myPricebookEntry =  myPricebookEntries.get(0);
		system.debug('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX myOppLine.myPricebookEntry.Product2.Administratie__c: '+ myPricebookEntry.Product2.Administratie__c + ' XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX');
		system.debug('****************************** myOppLine.PricebookEntry.Product2.Name : '+  myOppLine.PricebookEntry.Product2.Name +'*******************************');
    }
